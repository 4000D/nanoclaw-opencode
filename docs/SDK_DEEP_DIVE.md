> ⚠️ **DEPRECATED**: This document describes the previous Claude Agent SDK integration. NanoClaw now uses OpenCode (`@opencode-ai/sdk`). Kept for historical reference.

# Claude Agent SDK Deep Dive

Findings from reverse-engineering the previous Claude Agent SDK integration (v0.2.29-v0.2.34) to understand how `query()` works, why agent teams subagents were being killed, and how to fix it. Supplemented with official SDK reference docs.
