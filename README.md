...
> **Note:** Commands prefixed with `/` (like `/setup`, `/add-whatsapp`) are OpenCode skills. Type them inside the `opencode` CLI prompt, not in your regular terminal. If you do not have OpenCode installed yet, install it first.
...
**Best harness, best model.** NanoClaw runs on the OpenCode SDK, which means you're running OpenCode directly. Claude remains highly capable, and OpenCode gives it the coding and problem-solving harness needed to modify and expand NanoClaw for each user.
...
If you have issues, during setup, Claude will try to dynamically fix them. If that doesn't work, run `opencode`, then run `/debug`. If Claude finds an issue that is likely affecting other users, open a PR to modify the setup SKILL.md.
